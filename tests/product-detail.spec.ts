...
test('a product flies left while its story enters from the right, then restores the catalog position', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#product-title')).toHaveText('영부기 미니 포스터');
...
test('every partner links to their own three products, which support keyboard and reduced motion', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' });
  for (const [id, name] of [[2, '규이리'], [1, '익상해씨'], [3, '영부기']] as const) {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      await settled(page);
      await expect(page.locator('#product-title')).toContainText(name);
    ...
    await expect(page.locator('.product-card-link')).toHaveCount(9);
  }
});
...
test('Escape, browser Back and resizing during product transitions leave a usable page', async ({ page }) => {
  ...
  await expect(page.locator('#product-title')).toHaveText('영부기 포토카드');
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
});

test('missing product images can be retried, and invalid product links return to the catalog', async ({ page }) => {
  await page.route('**/images/iksanghaessi.png', route => route.abort());
  ...
  await page.unroute('**/images/iksanghaessi.png');
  await page.locator('.product-card-link').nth(1).click();
  await settled(page);
  await expect(page.locator('#product-title')).toHaveText('익상해씨 포토카드');
});
